...
\begin{frame}{Gegenbeispiele}
\begin{enumerate}
	\item $f\colon[0,1]\to[0,1]$ Cantorfunktion\dots\pause
	...
	\begin{equation*}
		g'(x)=\begin{cases}
			2x\sin(\frac{1}{x^2})-\frac{2}{x}\cos(\frac{1}{x^2}), &x\in(0,1],\\
			0, &x=0.
		\end{cases}
	\end{equation*}
...
\begin{frame}{Erinnerung - Lebesguesche Differentiation}
	\begin{definition}
		...
		\begin{equation*}
			E_i \subseteq B_x(r_i)\quad \text{und}\quad \abs{E_i}\geq \alpha\cdot\abs{B_x(r_i)} \:\forall\,i\in\NN.
		\end{equation*}
	\end{definition}
	\begin{theorem*}
		Sei $f\in L^1(\RR^d)$. Dann gilt
		\begin{equation*}
			f(x)=\lim_{i\to\infty} \frac{1}{\abs{E_i(x)}} \int_{E_i} f(t)\d t
		\end{equation*}
		für fast alle $x\in\RR^d$ und schön gegen $x$ schrumpfende Folgen $(E_i(x))_{i\in\NN}$.
	\end{theorem*}
\end{frame}
